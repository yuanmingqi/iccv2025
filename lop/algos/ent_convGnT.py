...
class EntConvGnT(object):
    ...
    def __init__(self, net, hidden_activation, opt, decay_rate=0.99, replacement_rate=1e-4, init='kaiming',
        ...
        super(EntConvGnT, self).__init__()
    ...
    def compute_entropy(self, feature_map, bins=50):
            feature_map = (feature_map - feature_map.min()) / (feature_map.max() - feature_map.min() + 1e-6)
            hist = torch.histc(feature_map, bins=bins, min=0.0, max=1.0)
            prob = hist / hist.sum()  
    ...
    def update_utility(self, layer_idx=0, features=None):
        with torch.no_grad():
            ...
            # get entropy factor
            ent_values = self.compute_entropy(features)
            ...
            new_util += self.lambda_ent * ent_values
